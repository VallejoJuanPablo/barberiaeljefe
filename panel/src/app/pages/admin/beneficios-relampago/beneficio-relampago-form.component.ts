...
@Component({
  selector: 'app-beneficio-relampago-form',
  standalone: true,
  ...
  template: `
    <div class="p-6 max-w-2xl">
      ...
      <div class="flex items-center gap-4 mb-6">
        <a routerLink="/admin/beneficios-relampago"
        ...
        <h1 class="text-2xl font-bold text-white">{{ isEdit ? 'Editar' : 'Nuevo' }} beneficio relámpago</h1>
      </div>
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-5 space-y-4">
          ...
          <div>
            ...
            <textarea [(ngModel)]="descripcion" name="descripcion" required rows="3"
                      ...
                      placeholder="Describí el beneficio en detalle..."></textarea>
          </div>
          ...
          <div class="flex items-center gap-3">
            ...
            <label for="activa" class="text-sm text-gray-300">Beneficio activo</label>
          </div>
        </div>
...
export class BeneficioRelampagoFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (id) {
      this.isEdit = true;
      this.editId = id;
      this.service.getById(id).subscribe(b => {
        this.titulo = b.titulo;
        this.descripcion = b.descripcion;
        this.fechaDesde = b.fechaDesde.substring(0, 10);
        this.fechaHasta = b.fechaHasta.substring(0, 10);
        this.activa = b.activa;
      });
    }
  }
  ...
  onSubmit() {
    ...
    obs.subscribe({
      next: () => this.router.navigate(['/admin/beneficios-relampago']),
      error: () => {
        this.loading = false;
